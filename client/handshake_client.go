...
import (
	...
	"selfproxy/cryptoProtect/zipper"
...
Once receiving ack, then generates a shakehand-Msg and divides it into two parts, and the first part will possess 50~60 percent.

Send each part sequentially while receiving the proxy-shakehandMsg and wait for corresponding ack and verify the validation.
...
func (c *Client) readStep1() error {
	...
	if err != nil {
		c.wNeedBytes <- []byte(``)
		c.MiProxy.CloseAll()
		return errors.New(`failed to receive (comp)ppub`)
	}
...
func (c *Client) writeStep3(cflow []byte, turn int) error {
	...
	if err != nil {
		c.MiProxy.CloseAll()
		return defErr.DescribeThenConcat(`failed to compress cflow:`, err)
	}
