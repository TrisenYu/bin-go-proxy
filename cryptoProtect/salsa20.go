// SPDX-LICENSE-IDENTIFIER: GPL-2.0-Only
// (C) 2024 Author: <kisfg@hotmail.com>
package cryptoprotect

import "golang.org/x/crypto/salsa20"

const GoogleCiphersNonceLen int = 24

type Salsa20 struct {
	Key      [KeySize]byte
	Iv       [IVSize]byte
	shadowIv [IVSize]byte
}

var sbox2 = [...]byte{
	,
}

func (s *Salsa20) DecryptFlow(msg []byte) ([]byte, error) {
	oup := make([]byte, len(msg))
	var extend_iv [GoogleCiphersNonceLen]byte
	copy(extend_iv[:IVSize], s.shadowIv[:])
	for i := IVSize; i < GoogleCiphersNonceLen; i++ {
		extend_iv[i] = s.shadowIv[i-IVSize] ^ s.shadowIv[i-IVSize/2]
	}
	salsa20.XORKeyStream(oup, msg, extend_iv[:], &s.Key)
	for i := 0; i < IVSize; i++ {
		tmp := (sbox2[s.shadowIv[i]] +
			s.shadowIv[i] + sbox2[s.shadowIv[(i-1+IVSize)%IVSize]]) & 0xFF
		s.shadowIv[i] = tmp
	}
	return oup, nil
}

func (s *Salsa20) EncryptFlow(msg []byte) ([]byte, error) {
	oup := make([]byte, len(msg))
	var extend_iv [GoogleCiphersNonceLen]byte
	copy(extend_iv[:IVSize], s.Iv[:])
	for i := IVSize; i < GoogleCiphersNonceLen; i++ {
		extend_iv[i] = s.Iv[i-IVSize] ^ s.Iv[i-IVSize/2]
	}
	// we need iv avalanche since salsa20 XorKeyStream always generates
	// the same ciphertext for the same plaintext if we don't change iv at all.
	salsa20.XORKeyStream(oup, msg, extend_iv[:], &s.Key)
	for i := 0; i < IVSize; i++ {
		tmp := byte(sbox2[s.Iv[i]] +
			s.Iv[i] + sbox2[s.Iv[(i-1+IVSize)%IVSize]])
		s.Iv[i] = tmp
	}
	return oup, nil
}

func (s *Salsa20) SetKey(key []byte) {
	s.Key = [KeySize]byte(key)
}

func (s *Salsa20) SetIv(iv []byte) {
	copy(s.Iv[:], iv[:IVSize])
	copy(s.shadowIv[:], iv[:IVSize])
}

func (s *Salsa20) GetKey() []byte {
	return s.Key[:]
}

func (s *Salsa20) GetIv() []byte {
	return s.Iv[:]
}
